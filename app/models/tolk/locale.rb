...
module Tolk
  class Locale < ActiveRecord::Base
    ...
    def phrases_without_translation(page = nil, options = {})
      ...
      opts = {:page => page, :per_page => Phrase.per_page}.merge(options)
      result = phrases.page(opts[:page]).per(opts[:per_page])
    ...
    def search_phrases(query, scope, key_query, page = nil, options = {})
      ...
      opts = {:page => page}.merge(options)
      phrases.page(opts[:page])
    end
    ...
    def find_phrases_with_translations(page, conditions = {})
      result = Tolk::Phrase.page(page).where({ :'tolk_translations.locale_id' => self.id }.merge(conditions)).joins(:translations).order('tolk_phrases.key ASC')
